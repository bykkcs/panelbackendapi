    ...
    <div className="grid md:grid-cols-2 gap-6">
      <form onSubmit={createOrder} className="bg-white p-4 rounded-2xl shadow space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            <input className={`border p-2 rounded w-full ${errors['client.name'] ? 'border-red-500' : ''}`} placeholder="ФИО" value={form.client.name} onChange={e=>setForm({...form, client:{...form.client, name:e.target.value}})} />
            <FieldError error={errors['client.name']} />
          </div>
          <div>
            <input className={`border p-2 rounded w-full ${errors['client.phone'] ? 'border-red-500' : ''}`} placeholder="Телефон" value={form.client.phone} onChange={e=>setForm({...form, client:{...form.client, phone:e.target.value}})} />
            <FieldError error={errors['client.phone']} />
          </div>
          <div className="col-span-2">
            <input className={`border p-2 rounded w-full ${errors['client.address'] ? 'border-red-500' : ''}`} placeholder="Адрес" value={form.client.address} onChange={e=>setForm({...form, client:{...form.client, address:e.target.value}})} />
            <FieldError error={errors['client.address']} />
          </div>
          <div>
            <input className={`border p-2 rounded w-full ${errors['client.metro'] ? 'border-red-500' : ''}`} placeholder="Метро" value={form.client.metro} onChange={e=>setForm({...form, client:{...form.client, metro:e.target.value}})} />
            <FieldError error={errors['client.metro']} />
          </div>
          <div>
            <input className={`border p-2 rounded w-full ${errors['client.howToGet'] ? 'border-red-500' : ''}`} placeholder="Как добраться" value={form.client.howToGet} onChange={e=>setForm({...form, client:{...form.client, howToGet:e.target.value}})} />
            <FieldError error={errors['client.howToGet']} />
          </div>
          <div>
            <input className={`border p-2 rounded w-full ${errors['applianceType'] ? 'border-red-500' : ''}`} placeholder="Вид техники" value={form.applianceType} onChange={e=>setForm({...form, applianceType:e.target.value})} />
            <FieldError error={errors['applianceType']} />
          </div>
          <div>
            <input className={`border p-2 rounded w-full ${errors['applianceAge'] ? 'border-red-500' : ''}`} placeholder="Возраст техники" value={form.applianceAge} onChange={e=>setForm({...form, applianceAge:e.target.value})} />
            <FieldError error={errors['applianceAge']} />
          </div>
          <div className="col-span-2">
            <input className={`border p-2 rounded w-full ${errors['problem'] ? 'border-red-500' : ''}`} placeholder="Описание проблемы" value={form.problem} onChange={e=>setForm({...form, problem:e.target.value})} />
            <FieldError error={errors['problem']} />
          </div>
          <div>
            <input type="datetime-local" className={`border p-2 rounded w-full ${errors['scheduledAt'] ? 'border-red-500' : ''}`} value={form.scheduledAt} onChange={e=>setForm({...form, scheduledAt:e.target.value})} />
